...
from .models import LostandFound,Picture,Comment
...
def uploadpost_lostf(request):
    ...
    if(request.method=="POST"):
        ...
        try:
            ...
            post_lostf=LostandFound(user=user,title=title,content=content,itemtype=itemtype,like_count=0)
            post_lostf.save()
            for i in range(len(imgurls)):
                img=Picture(img_url=imgurls[i],post=post_lostf)
                img.save()
            response["msg"]="true"
            response["post_id"]=post_lostf.id
            response["post_time"]=post_lostf.created_time
...
'''
@csrf_exempt #上传图片
def uploadimg(request,arg):
    response={}
    response["msg"] = "true"
    if(request.method=="POST"):
        try:
            post_lostf=LostandFound.objects.get(id=arg)
            img=Picture(post=post_lostf,img_url=request.FILES.get('img'))
            print(img.img_url)
...
def uploadcomment(request):
    if(request.method=="POST"):
        ...
        to_which_post=LostandFound.objects.get(id=req["postid"])
        to_which_user=None
        try:
...
def getpost(request):
    if(request.method=="POST"):
        response={}
        #逆排序
        posts=list(LostandFound.objects.all().order_by("-created_time"))
        i=1
        for post in posts:
           postname="post"+str(i)
           j = 1
...
def getpostcomment(request):
    ...
    if request.method=="POST":
        req=simplejson.loads(request.body)
        postid=req["postid"]
        post=LostandFound.objects.get(id=postid)
...
def add_likecount(request):
    if(request.method=="POST"):
        ...
        req=simplejson.loads(request.body)
        postid=req["post_id"]
        post_lostf=LostandFound.objects.get(id=postid)
